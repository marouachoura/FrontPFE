...
export class PublicationsService {
  ...
  savePub(pub: Publications): Promise<Publications> {
    return this.httpClient.post<Publications>('http://localhost:8080/formations/add', pub).toPromise();
  ...
  getPubById(id: string): Promise<Publications> {
    return this.httpClient.get<Publications>('http://localhost:8080/formations/find/' + id).toPromise();
    /*return new Promise(resolve => resolve(
      this.tab.filter(item => item.id===id)[0]??null));*/
  }

  RemovePubById(id: string): Promise<void> {
    return this.httpClient.delete<void>('http://localhost:8080/formations/delete/' + id).toPromise();
  ...
  GetALL(): Promise<Publications[]> {
    return this.httpClient.get<any[]>('http://localhost:8080/formations').toPromise();

  }
  EditPub(id: any, pub: Publications): Promise<Publications> {
    return this.httpClient.put<Publications>('http://localhost:8080/formation/update/' + id, pub).toPromise();
